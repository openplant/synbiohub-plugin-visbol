 ...
 */
GraphEdits.interactionEdit = class {
  /**
   *
...
GraphEdits.infoEdit = class {
  ...
  constructor(
    ...
    info,
    previousInfo,
    dictionaryIndex = graph_base_js_1.GraphBase.INFO_DICT_INDEX
  ) {
    this.cell0 = cell0
...
GraphEdits.zoomEdit = class {
   ...
   */
  constructor(view, glyphCell, graphService) {
    this.view = view
  ...
  execute() {
    if (this.glyphCell != null) {
      ...
      else this.graphService.graph.clearSelection()
      this.graphService.fitCamera()
      // make sure we can't add new strands/interactions/molecules
      this.graphService.setComponentDefinitionMode(
        this.graphService.graph.getCurrentRoot().isComponentView()
      )
      this.glyphCell = null
    } else {
      ...
      if (
        this.graphService.graph.getCurrentRoot() &&
        this.graphService.graph.getCurrentRoot().isViewCell()
      ) {
        this.graphService.setComponentDefinitionMode(
          this.graphService.graph.getCurrentRoot().isComponentView()
        )
