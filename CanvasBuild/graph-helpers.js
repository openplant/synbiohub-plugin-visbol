...
class GraphHelpers extends graph_base_js_1.GraphBase {
  constructor(glyphService, metadataService) {
    ...
    const rootViewCell = this.graph.insertVertex(
      ...
      0,
      0,
      graph_base_js_1.GraphBase.STYLE_MODULE_VIEW
    )
  ...
  updateSelectedGlyphInfo(info) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (
          oldGlyphInfo.uriPrefix != environment_js_1.environment.baseURI &&
          !(yield this.promptMakeEditableCopy(oldGlyphInfo.displayID))
        ) {
          return
        }
        if (oldGlyphURI != newGlyphURI) {
          ...
          if (selectedCell.isCircuitContainer() || selectedCell.isViewCell()) {
            if (this.graph.getCurrentRoot().isComponentView()) {
              ...
              this.graph
                .getModel()
                .execute(new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), null, this))
            } else {
          ...
          if (!shouldDecouple) {
            ...
            if (glyphZoomed) {
              if (glyphZoomed.isViewCell()) {
                ...
                this.graph
                  .getModel()
                  .execute(
                    new graph_edits_js_1.GraphEdits.zoomEdit(
                      this.graph.getView(),
                      newViewCell,
                      this
                    )
            ...
            this.graph
              .getModel()
              .execute(
                new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), glyphZoomed, this)
              )
          } else {
        ...
        this.mutateSequenceFeatureGlyph(
          this.getFromInfoDict(selectedCell.value).partRole,
          ...
          if (!shouldDecouple) {
            ...
            if (moduleZoomed.isViewCell()) {
              ...
              this.graph
                .getModel()
                .execute(
                  new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), newViewCell, this)
                )
            } else {
              ...
              this.graph
                .getModel()
                .execute(
                  new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), moduleZoomed, this)
                )
            }
          } else {
            // if we want to decouple, just set the current cell
  ...
  updateSelectedCombinatorialInfo(info, prevURI) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (
          info.uriPrefix != environment_js_1.environment.baseURI &&
          !(yield this.promptMakeEditableCopy(info.displayID))
        ) {
          return
        }
        ...
        for (let i = this.selectionStack.length - 1; i >= 0; i--) {
          ...
          if (!varCompInfo) {
            ...
            varCompInfo = new variableComponentInfo_js_1.VariableComponentInfo(
              this.selectionStack[i].getId()
            )
  ...
  mutateSequenceFeatureGlyph(name, cells, graph) {
    ...
    try {
      ...
      if (!cellStyle.fillColor)
        newStyleName =
          graph_base_js_1.GraphBase.STYLE_SEQUENCE_FEATURE + graph_base_js_1.GraphBase.STYLE_NGA
  ...
  makeGeneralDragsource(element, insertFunc) {
    ...
    const ds = graph_base_js_1.mx.mxUtils.makeDraggable(
      element,
      this.graph,
      insertFunc,
      null,
  ...
  changeOwnership(infoURI, fullCheck = false) {
    ...
    try {
      ...
      if (
        !info.uriPrefix ||
        (info.uriPrefix === environment_js_1.environment.baseURI && !fullCheck)
      ) {
        return
      }
      ...
      while (toCheck.size > 0) {
        ...
        if (!combinatorialMode) {
          ...
          for (let key in cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX]) {
            const combinatorial =
              cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX][key]
      ...
      if (!combinatorialMode) {
        ...
        if (rootViewInfo) {
          ...
          this.graph
            .getModel()
            .execute(
              new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), newRootViewCell, this)
            )
        }
        ...
        for (let i = 0; i < zoomedCells.length; i++) {
          this.graph
            .getModel()
            .execute(
              new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), zoomedCells[i], this)
            )
        }
      }
    } finally {
  ...
  updateInfoDict(info) {
    ...
    this.graph
      .getModel()
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[graph_base_js_1.GraphBase.INFO_DICT_INDEX][info.getFullURI()]
        )
      )
  }
  /**
    ...
    this.graph
      ...
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          null,
          cell0.value[graph_base_js_1.GraphBase.INFO_DICT_INDEX][glyphURI]
        )
      )
  }
  /**
    ...
    this.graph
      ...
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX][info.getFullURI()],
          graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX
        )
      )
  }
  /**
    ...
    this.graph
      ...
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          null,
          cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX][glyphURI],
          graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX
        )
      )
  }
  ...
  getCombinatorialWithTemplate(templateURI) {
    ...
    for (let key in cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX]) {
      if (
        cell0.value[graph_base_js_1.GraphBase.COMBINATORIAL_DICT_INDEX][key].templateURI ===
        templateURI
      ) {
    ...
    this.graph
      ...
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[graph_base_js_1.GraphBase.INTERACTION_DICT_INDEX][info.getFullURI()],
          graph_base_js_1.GraphBase.INTERACTION_DICT_INDEX
        )
      )
  }
    ...
    this.graph
      ...
      .execute(
        new graph_edits_js_1.GraphEdits.infoEdit(
          cell0,
          null,
          cell0.value[graph_base_js_1.GraphBase.INTERACTION_DICT_INDEX][interactionURI],
          graph_base_js_1.GraphBase.INTERACTION_DICT_INDEX
        )
      )
  }
  ...
  initLabelDrawing() {
    ...
    this.graph.getLabel = function (cell) {
      ...
      if (label) {
        ...
        let fontSize = graphService.graph.getCellStyle(cell)[
          graph_base_js_1.mx.mxConstants.STYLE_FONTSIZE
        ]
    ...
    graph_base_js_1.mx.mxGraphView.prototype.getPerimeterPoint = function (
      terminal,
      next,
      orthogonal,
      border
  ...
  createViewCell(uri, module = false) {
    ...
    if (module) {
      ...
      childViewCell = this.graph.insertVertex(
        ...
        0,
        0,
        graph_base_js_1.GraphBase.STYLE_COMPONENT_VIEW
      )
      ...
      const childCircuitContainer = this.graph.insertVertex(
        ...
        0,
        0,
        graph_base_js_1.GraphBase.STYLE_CIRCUIT_CONTAINER
      )
      const childCircuitContainerBackbone = this.graph.insertVertex(
        ...
        0,
        0,
        graph_base_js_1.GraphBase.STYLE_BACKBONE
      )
