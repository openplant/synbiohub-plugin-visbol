...
class GraphService extends graph_helpers_js_1.GraphHelpers {
  constructor(glyphService, metadataService) {
    ...
    this.graph
      .getSelectionModel()
      .addListener(
        graph_base_js_1.mx.mxEvent.CHANGE,
        graph_base_js_1.mx.mxUtils.bind(this, this.handleSelectionChange)
      )
  }
  isSelectedAGlyph() {
  ...
  isInteractionTypeAllowed(interactionType) {
    ...
    if (
      selected.length > 1 ||
      selected.length == 0 ||
      (!selected[0].isInteraction() && !selected[0].isInteractionNode)
    ) {
      return false
    }
    ...
    if (selected[0].isInteractionNode()) {
      return (
        interactionType == 'Biochemical Reaction' ||
        interactionType == 'Dissociation' ||
        interactionType == 'Genetic Production' ||
        interactionType == 'Non-Covalent Binding'
      )
    }
    return false
  }
  ...
  flipSequenceFeatureGlyph() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      for (let parentInfo of Array.from(parentInfos.values())) {
        if (
          parentInfo.uriPrefix != environment_js_1.environment.baseURI &&
          !(yield this.promptMakeEditableCopy(parentInfo.displayID))
        ) {
          return
        }
      }
      try {
        ...
        for (let cell of selectionCells) {
          if (cell.isSequenceFeatureGlyph()) {
            ...
            let direction = this.graph.getCellStyle(cell)[
              graph_base_js_1.mx.mxConstants.STYLE_DIRECTION
            ]
            ...
            if (direction == undefined) {
              ...
              this.graph.setCellStyles(graph_base_js_1.mx.mxConstants.STYLE_DIRECTION, 'west', [
                cell,
              ])
              ...
              this.graph.setCellStyles(graph_base_js_1.mx.mxConstants.STYLE_DIRECTION, 'east', [
                cell,
              ])
  ...
  enterGlyph() {
    ...
    try {
      ...
      let zoomEdit = new graph_edits_js_1.GraphEdits.zoomEdit(
        this.graph.getView(),
        selection[0],
        this
      )
  ...
  makeBackboneDragsource(element) {
    const insertGlyph = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addBackboneAt(
        x - graph_base_js_1.GraphBase.sequenceFeatureGlyphWidth / 2,
        y - graph_base_js_1.GraphBase.sequenceFeatureGlyphHeight / 2
      )
    })
  ...
  addBackboneAt(x, y) {
    ...
    try {
      ...
      const circuitContainer = this.graph.insertVertex(
        this.graph.getDefaultParent(),
        null,
        glyphInfo.getFullURI(),
        x,
        y,
        graph_base_js_1.GraphBase.sequenceFeatureGlyphWidth,
        graph_base_js_1.GraphBase.sequenceFeatureGlyphHeight,
        graph_base_js_1.GraphBase.STYLE_CIRCUIT_CONTAINER
      )
      const backbone = this.graph.insertVertex(
        circuitContainer,
        null,
        '',
        0,
        graph_base_js_1.GraphBase.sequenceFeatureGlyphHeight / 2,
        graph_base_js_1.GraphBase.sequenceFeatureGlyphWidth,
        1,
        ...
        for (let cell of selectedCells) {
          if (cell.isSequenceFeatureGlyph()) {
            ...
            if (cell.isSequenceFeatureGlyph()) {
              ...
              if (combinatorial) combinatorial.removeVariableComponentInfo(cell.getId())
            }
          } else if (
            cell.isCircuitContainer() &&
            this.graph.getCurrentRoot() &&
            this.graph.getCurrentRoot().isComponentView()
          )
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        const sequenceFeatureCell = this.graph.insertVertex(
          ...
          graph_base_js_1.GraphBase.sequenceFeatureGlyphWidth,
          graph_base_js_1.GraphBase.sequenceFeatureGlyphHeight,
          graph_base_js_1.GraphBase.STYLE_SEQUENCE_FEATURE + name
        )
  ...
  makeMolecularSpeciesDragsource(element, stylename) {
    const insertGlyph = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addMolecularSpeciesAt(
        stylename,
        x - graph_base_js_1.GraphBase.molecularSpeciesGlyphWidth / 2,
        y - graph_base_js_1.GraphBase.molecularSpeciesGlyphHeight / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
  ...
  addMolecularSpeciesAt(name, x, y) {
    ...
    try {
      ...
      const molecularSpeciesGlyph = this.graph.insertVertex(
        this.graph.getDefaultParent(),
        null,
        proteinInfo.getFullURI(),
        x,
        y,
        graph_base_js_1.GraphBase.molecularSpeciesGlyphWidth,
        graph_base_js_1.GraphBase.molecularSpeciesGlyphHeight,
        graph_base_js_1.GraphBase.STYLE_MOLECULAR_SPECIES + name
      )
  ...
  makeInteractionNodeDragsource(element, stylename) {
    const insertGlyph = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addInteractionNodeAt(
        stylename,
        x - graph_base_js_1.GraphBase.interactionNodeGlyphWidth / 2,
        y - graph_base_js_1.GraphBase.interactionNodeGlyphHeight / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
  ...
  addInteractionNodeAt(name, x, y) {
    ...
    try {
      ...
      const interactionNodeGlyph = this.graph.insertVertex(
        this.graph.getDefaultParent(),
        null,
        interactionInfo.getFullURI(),
        x,
        y,
        graph_base_js_1.GraphBase.interactionNodeGlyphWidth,
        graph_base_js_1.GraphBase.interactionNodeGlyphHeight,
        graph_base_js_1.GraphBase.STYLE_INTERACTION_NODE + name
      )
  ...
  makeInteractionDragsource(element, stylename) {
    const insertGlyph = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addInteractionAt(
        stylename,
        x - graph_base_js_1.GraphBase.defaultInteractionSize / 2,
        y - graph_base_js_1.GraphBase.defaultInteractionSize / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
  ...
  addInteraction(name) {
    ...
    if (selectionCells.length > 0 && selectionCells.length < 3) {
      ...
      if (selectedParent.geometry && !selectedParent.isViewCell()) {
        this.addInteractionAt(
          name,
          selectedParent.geometry.x + selectedCell.geometry.x + selectedCell.geometry.width / 2,
          selectedParent.geometry.y
        )
      } else {
        this.addInteractionAt(
          name,
          selectedCell.geometry.x + selectedCell.geometry.width / 2,
  ...
  addInteractionAt(name, x, y) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        let interactionInfo = new interactionInfo_js_1.InteractionInfo()
        cell = new graph_base_js_1.mx.mxCell(
          interactionInfo.getFullURI(),
          new graph_base_js_1.mx.mxGeometry(x, y, 0, 0),
          graph_base_js_1.GraphBase.STYLE_INTERACTION + name
        )
        const selectionCells = this.graph.getSelectionCells()
        if (selectionCells.length == 1) {
          ...
          cell.geometry.setTerminalPoint(
            new graph_base_js_1.mx.mxPoint(x, y - graph_base_js_1.GraphBase.defaultInteractionSize),
            false
          )
          cell.edge = true
          ...
          cell.geometry.setTerminalPoint(
            new graph_base_js_1.mx.mxPoint(x + graph_base_js_1.GraphBase.defaultInteractionSize, y),
            false
          )
          cell.edge = true
  ...
  makeTextboxDragsource(element) {
    const insert = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addTextBoxAt(
        x - graph_base_js_1.GraphBase.defaultTextWidth / 2,
        y - graph_base_js_1.GraphBase.defaultTextHeight / 2
      )
    })
  ...
  addTextBoxAt(x, y) {
    ...
    try {
      const cell = this.graph.insertVertex(
        ...
        x,
        y,
        graph_base_js_1.GraphBase.defaultTextWidth,
        graph_base_js_1.GraphBase.defaultTextHeight,
        graph_base_js_1.GraphBase.STYLE_TEXTBOX
      )
  ...
  makeModuleDragsource(element) {
    const insert = graph_base_js_1.mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addModuleAt(
        x - graph_base_js_1.GraphBase.defaultModuleWidth / 2,
        y - graph_base_js_1.GraphBase.defaultModuleHeight / 2
      )
    })
    this.makeGeneralDragsource(element, insert)
  }
  ...
  addModuleAt(x, y) {
    ...
    try {
      ...
      const moduleCell = this.graph.insertVertex(
        this.graph.getDefaultParent(),
        null,
        moduleInfo.getFullURI(),
        x,
        y,
        graph_base_js_1.GraphBase.defaultModuleWidth,
        graph_base_js_1.GraphBase.defaultModuleHeight,
        graph_base_js_1.GraphBase.STYLE_MODULE
      )
  ...
  setSelectedCellInfo(info) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (
          info instanceof moduleInfo_js_1.ModuleInfo &&
          (!selectedCell || selectedCell.isModule())
        ) {
          this.updateSelectedModuleInfo(info)
          return
        }
        if (
          info instanceof glyphInfo_js_1.GlyphInfo &&
          (!selectedCell ||
            selectedCell.isSequenceFeatureGlyph() ||
            selectedCell.isCircuitContainer() ||
            selectedCell.isMolecularSpeciesGlyph())
        ) {
        ...
        if (
          info instanceof interactionInfo_js_1.InteractionInfo &&
          (selectedCell.isInteraction() || selectedCell.isInteractionNode())
        ) {
  ...
  setSelectedCombinatorialInfo(info, prevURI) {
    ...
    try {
      if (
        info instanceof combinatorialInfo_js_1.CombinatorialInfo &&
        selectedCell.isSequenceFeatureGlyph()
      ) {
        if (!prevURI) {
  ...
  exportSVG(filename) {
    ...
    var root =
      svgDoc.createElementNS != null
        ? svgDoc.createElementNS(graph_base_js_1.mx.mxConstants.NS_SVG, 'svg')
    ...
    if (svgDoc.createElementNS == null) {
      ...
      root.setAttributeNS(
        ...
        'xmlns:xlink',
        graph_base_js_1.mx.mxConstants.NS_XLINK
      )
    }
    ...
    var group =
      svgDoc.createElementNS != null
        ? svgDoc.createElementNS(graph_base_js_1.mx.mxConstants.NS_SVG, 'g')
  ...
  setGraphToXML(graphString) {
    ...
    if (children) {
      children.forEach((element) => {
        if (element.isCircuitContainer()) element.refreshCircuitContainer(this.graph)
      })
    }
        ...
        try {
          ...
          if (parentInfo) {
            if (selectedCell.isViewCell()) {
              ...
              let selectedIndex = selectedCell.getParent().getIndex(selectedCell)
              this.changeOwnership(parentInfo.getFullURI())
              selectedCell = this.graph.getCurrentRoot().children[parentIndex].children[
                selectedIndex
              ]
            }
          }
          ...
          let zoomOut = false
          if (
            ((selectedCell.isCircuitContainer() || selectedCell.isComponentView()) &&
              this.graph.getCurrentRoot().isComponentView() &&
          ...
          for (let i = 1; i < viewCells.length; i++) {
            // start at cell 1 because the glyph is at 0
            ...
            this.addToInfoDict(
              subGlyphDict[graph_base_js_1.GraphBase.INFO_DICT_INDEX][viewCells[i].getId()]
            )
            ...
            for (let child of viewClone.children) {
              if (child.isMolecularSpeciesGlyph()) {
                ...
                this.addToInfoDict(
                  subGlyphDict[graph_base_js_1.GraphBase.INFO_DICT_INDEX][child.value]
                )
              } else if (child.isInteractionNode()) {
                if (this.getFromInteractionDict(child.value) != null) {
                  this.removeFromInteractionDict(child.value)
                }
                this.addToInteractionDict(
                  subGlyphDict[graph_base_js_1.GraphBase.INTERACTION_DICT_INDEX][child.value]
                )
              } else if (child.isInteraction()) {
                // if either end is an interaction node, we don't need to bother
                if (
                  (child.source && child.source.isInteractionNode()) ||
                  (child.target && child.target.isInteractionNode())
                ) {
                  continue
                }
                if (this.getFromInteractionDict(child.value) != null) {
              ...
              if (child.isInteractionNode()) {
                ...
                for (let k = 0; k < child.edges.length; k++) {
                  ...
                  if (infoCopy.toURI[originalEdge.getId()]) {
                    ...
                    let oldCell = subGraph
                      .getModel()
                      .getCell(cellRef.substring(cellRef.lastIndexOf('_') + 1))
                    ...
                    newTo[edge.getId()] =
                      cellRef.substring(0, cellRef.lastIndexOf('_') + 1) +
                      newParentView.children[oldParentView.getIndex(oldCell)].getId()
                  }
          ...
          if (zoomOut) {
            if (selectedCell.isSequenceFeatureGlyph()) {
              ...
              this.graph
                .getModel()
                .execute(
                  new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), newCell, this)
                )
            } else {
              // if it isn't, that means we are in a module, or at the root
              let newRootView = this.graph.getModel().getCell(newCell.getValue())
              let circuitContainer = this.graph
                .getModel()
                .filterCells(newRootView.children, (cell) => cell.isCircuitContainer())[0]
              if (inModuleView) {
                ...
                circuitContainer = this.graph
                  .getModel()
                  .add(selectedCell.getParent(), circuitContainer)
                this.graph.getModel().remove(selectedCell)
                circuitContainer.refreshCircuitContainer(this.graph)
              } else {
          ...
          if (selectedCell.isViewCell()) {
            ...
            this.graph
              .getModel()
              .execute(
                new graph_edits_js_1.GraphEdits.zoomEdit(this.graph.getView(), newView, this)
              )
          ...
          if (selectedCell.isCircuitContainer()) {
            ...
            this.updateInteractions(
              previousReference,
              newCell.getValue() + '_' + selectedCell.getId()
            )
          }
          if (graph_base_js_1.GraphBase.unFormatedCells.size > 0) {
            console.log('FORMATTING !!!!!!!!!!!!!!!!')
  ...
  resetGraph(moduleMode = true) {
    ...
    if (moduleMode) {
      ...
      rootViewCell = this.graph.insertVertex(
        cell1,
