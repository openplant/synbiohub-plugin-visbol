...
export class GraphService extends GraphHelpers {
  constructor(glyphService: GlyphService, metadataService: MetadataService) {
    ...
    this.graph
      .getSelectionModel()
      .addListener(mx.mxEvent.CHANGE, mx.mxUtils.bind(this, this.handleSelectionChange))
  }
  ...
  async flipSequenceFeatureGlyph() {
    ...
    for (let parentInfo of Array.from(parentInfos.values())) {
      if (
        parentInfo.uriPrefix != environment.baseURI &&
        !(await this.promptMakeEditableCopy(parentInfo.displayID))
      ) {
        return
      }
    }
  ...
  makeBackboneDragsource(element) {
    const insertGlyph = mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addBackboneAt(
        x - GraphBase.sequenceFeatureGlyphWidth / 2,
        y - GraphBase.sequenceFeatureGlyphHeight / 2
      )
    })
  ...
  addBackboneAt(x, y) {
    ...
    try {
      ...
      const circuitContainer = this.graph.insertVertex(
        ...
        x,
        y,
        GraphBase.sequenceFeatureGlyphWidth,
    ...
    if (this.graph.getCurrentRoot()) {
      ...
      for (let container of Array.from(containers.values())) {
        ...
        if (
          ...
          !(await this.promptMakeEditableCopy(glyphInfo.displayID))
        ) {
          return
        }
      }
  ...
  async addSequenceFeatureAt(name, x, y, circuitContainer?) {
    ...
    try {
      ...
      const sequenceFeatureCell = this.graph.insertVertex(
        ...
        GraphBase.sequenceFeatureGlyphWidth,
        GraphBase.sequenceFeatureGlyphHeight,
        GraphBase.STYLE_SEQUENCE_FEATURE + name
      )
  ...
  makeMolecularSpeciesDragsource(element, stylename) {
    const insertGlyph = mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addMolecularSpeciesAt(
        stylename,
        x - GraphBase.molecularSpeciesGlyphWidth / 2,
        y - GraphBase.molecularSpeciesGlyphHeight / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
  ...
  addMolecularSpeciesAt(name, x, y) {
    ...
    try {
      ...
      const molecularSpeciesGlyph = this.graph.insertVertex(
        ...
        x,
        y,
        GraphBase.molecularSpeciesGlyphWidth,
        GraphBase.molecularSpeciesGlyphHeight,
        GraphBase.STYLE_MOLECULAR_SPECIES + name
      )
  ...
  makeInteractionNodeDragsource(element, stylename) {
    const insertGlyph = mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addInteractionNodeAt(
        stylename,
        x - GraphBase.interactionNodeGlyphWidth / 2,
        y - GraphBase.interactionNodeGlyphHeight / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
  ...
  addInteractionNodeAt(name: string, x, y) {
    ...
    try {
      ...
      const interactionNodeGlyph = this.graph.insertVertex(
        ...
        x,
        y,
        GraphBase.interactionNodeGlyphWidth,
        GraphBase.interactionNodeGlyphHeight,
  ...
  makeInteractionDragsource(element, stylename) {
    const insertGlyph = mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
      this.addInteractionAt(
        stylename,
        x - GraphBase.defaultInteractionSize / 2,
        y - GraphBase.defaultInteractionSize / 2
      )
    })
    this.makeGeneralDragsource(element, insertGlyph)
  }
    ...
    try {
      ...
      if (selectionCells.length == 1) {
        ...
        cell.geometry.setTerminalPoint(
          new mx.mxPoint(x, y - GraphBase.defaultInteractionSize),
          false
        )
        cell.edge = true
        ...
        cell.geometry.setTerminalPoint(
          new mx.mxPoint(x + GraphBase.defaultInteractionSize, y),
          false
        )
        cell.edge = true
  ...
  makeTextboxDragsource(element) {
    const insert = mx.mxUtils.bind(this, (graph, evt, target, x, y) => {
  ...
  addTextBoxAt(x, y) {
    ...
    try {
      const cell = this.graph.insertVertex(
        ...
        x,
        y,
        GraphBase.defaultTextWidth,
        GraphBase.defaultTextHeight,
        GraphBase.STYLE_TEXTBOX
      )
  ...
  addModuleAt(x, y) {
    ...
    try {
      ...
      const moduleCell = this.graph.insertVertex(
        ...
        x,
        y,
        GraphBase.defaultModuleWidth,
        GraphBase.defaultModuleHeight,
        GraphBase.STYLE_MODULE
      )
  ...
  async setSelectedCellInfo(info: Info) {
    ...
    try {
      ...
      if (
        info instanceof InteractionInfo &&
      ...
      try {
        ...
        if (zoomOut) {
          if (selectedCell.isSequenceFeatureGlyph()) {
            ...
            this.graph
              .getModel()
              .execute(new GraphEdits.zoomEdit(this.graph.getView(), newCell, this))
          } else {
            ...
            if (inModuleView) {
              ...
              this.graph
                .getModel()
                .execute(new GraphEdits.zoomEdit(this.graph.getView(), newRootView, this))
              circuitContainer.refreshCircuitContainer(this.graph)
            }
          }
        }
        ...
        if (selectedCell.isViewCell()) {
          ...
          this.graph
            .getModel()
            .execute(new GraphEdits.zoomEdit(this.graph.getView(), newView, this))
