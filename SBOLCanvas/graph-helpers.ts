...
export class GraphHelpers extends GraphBase {
    ...
    protected async promptChooseFunctionalComponent(cell: typeof mxCell, from: boolean): Promise<string> {
        let viewCell;
        if (cell.isModule()) {
   ...
   */
    ...
    protected makeGeneralDragsource(element, insertFunc) {
        ...
        const ds: typeof mxDragSource = mx.mxUtils.makeDraggable(element, this.graph, insertFunc, null, xOffset, yOffset);
 ...
 */
    ...
    protected getCoupledCircuitContainers(glyphURI: string, onlyModuleContainers: boolean = false): typeof mxCell[] {
        const coupledCells = [];
        const cell1 = this.graph.getModel().getCell("1");
        for (let viewCell of cell1.children) {
