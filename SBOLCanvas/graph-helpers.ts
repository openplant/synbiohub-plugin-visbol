...
export class GraphHelpers extends GraphBase {
  constructor(glyphService: GlyphService, protected metadataService: MetadataService) {
    ...
    const rootViewCell = this.graph.insertVertex(
      cell1,
      rootModuleInfo.getFullURI(),
      '',
      0,
      0,
      0,
      0,
      GraphBase.STYLE_MODULE_VIEW
    )
  ...
  protected getParentInfo(cell: typeof mxCell): GlyphInfo {
    let glyphInfo
    if (cell.isCircuitContainer() || cell.isModule()) {
      ...
      if (this.viewStack.length > 1) {
        ...
        glyphInfo = this.getFromInfoDict(
          this.selectionStack[this.selectionStack.length - 1].getParent().getValue()
        )
      }
    } else {
  ...
  protected async updateSelectedGlyphInfo(this: GraphService, info: GlyphInfo) {
    ...
    try {
      ...
      if (
        oldGlyphInfo.uriPrefix != environment.baseURI &&
        !(await this.promptMakeEditableCopy(oldGlyphInfo.displayID))
      ) {
        return
      }
      ...
      if (oldGlyphURI != newGlyphURI) {
        ...
        if (this.checkCycleUp(selectedCell, newGlyphURI)) {
          ...
          console.log(
            'ComponentInstance objects MUST NOT form circular reference chains via their definition properties and parent ComponentDefinition objects.'
          )
          return
        }
        ...
        for (let coupledCell of coupledCells) {
          if (
            coupledCell != selectedCell &&
            (selectedCell.isSequenceFeatureGlyph() ||
              ((selectedCell.isCircuitContainer() || selectedCell.isViewCell()) &&
                this.selectionStack.length > 0 &&
                coupledCell != this.selectionStack[this.selectionStack.length - 1]))
          ) {
            promptDecouple = true
            break
          }
        ...
        if (!promptDecouple) {
          for (let coupledContainer of coupledContainers) {
            if (
              coupledContainer != selectedCell &&
              this.getParentInfo(coupledContainer) &&
              this.getParentInfo(selectedCell) &&
              this.getParentInfo(coupledContainer).getFullURI() !=
                this.getParentInfo(selectedCell).getFullURI()
            ) {
              promptDecouple = true
              break
            }
        ...
        if (!shouldDecouple) {
          ...
          if (glyphZoomed) {
            if (glyphZoomed.isViewCell()) {
              ...
              this.graph
                .getModel()
                .execute(new GraphEdits.zoomEdit(this.graph.getView(), newViewCell, this))
            } else {
              this.graph
                .getModel()
                .execute(new GraphEdits.zoomEdit(this.graph.getView(), glyphZoomed, this))
            }
          }
        } else if (glyphZoomed) {
        ...
        if (selectedCell.isCircuitContainer()) {
          if (!shouldCouple || keepSubstructure) {
            ...
            if (conflictViewCell) {
              ...
              for (let circuitContainer of conflictViewCell.children) {
                if (!circuitContainer.isCircuitContainer()) continue
          ...
          if (!viewCell) viewCell = this.createViewCell(newGlyphURI)
          if (!shouldCouple || keepSubstructure) {
            ...
            for (let viewCell of cell1.children) {
              if (viewCell.getId() != newGlyphURI) {
                for (let viewChild of viewCell.children) {
                  if (!viewChild.isCircuitContainer()) continue
                  if (viewChild.getValue() === newGlyphURI) {
                    circuitContainer = viewChild
                    break
                  }
                }
                if (circuitContainer) break
              }
            }
        ...
        this.updateInteractions(
          oldGlyphURI + '_' + selectedCell.getId(),
          newGlyphURI + '_' + selectedCell.getId()
        )
      ...
      this.mutateSequenceFeatureGlyph(
        (<GlyphInfo>this.getFromInfoDict(selectedCell.value)).partRole,
        this.getCoupledGlyphs(selectedCell.value)
      )
    ...
    try {
      ...
      if (oldModuleURI != newModuleURI) {
        if (this.checkCycleUp(selectedCell, newModuleURI)) {
          // Tell the user a cycle isn't allowed
          console.log(
            'ModuleInstance objects MUST NOT form circular reference chains via their definition properties and parent ModuleDefinition objects.'
          )
          return
        }
        ...
        for (let coupledCell of coupledModules) {
          if (
            coupledCell != selectedCell &&
            (selectedCell.isModule() ||
              (selectedCell.isViewCell() &&
                this.selectionStack.length > 0 &&
                coupledCell != this.selectionStack[this.selectionStack.length - 1]))
          ) {
            promptDecouple = true
        ...
        if (!shouldDecouple) {
          ...
          if (moduleZoomed) {
            if (moduleZoomed.isViewCell()) {
              ...
              this.graph
                .getModel()
                .execute(new GraphEdits.zoomEdit(this.graph.getView(), moduleZoomed, this))
            }
          }
  ...
  protected async updateSelectedCombinatorialInfo(
    this: GraphService,
    info: CombinatorialInfo,
    prevURI: string
  ) {
    this.graph.getModel().beginUpdate()
    try {
      ...
      if (
        info.uriPrefix != environment.baseURI &&
        !(await this.promptMakeEditableCopy(info.displayID))
      ) {
        return
      }
      ...
      if (conflictCombinatorial && conflictCombinatorial.templateURI != info.templateURI) {
        console.log(
          'The part ' +
            info.getFullURI() +
            ' already exists as a Combinatorial, and points to a different Component Definition!'
        )
      }

      // store
      if (prevURI != info.getFullURI()) {
        if (this.getFromCombinatorialDict(prevURI)) this.removeFromCombinatorialDict(prevURI)
  ...
  protected updateSelectedInteractionInfo(this: GraphService, info: InteractionInfo) {
    ...
    for (let viewChild of cell1.children) {
      if (viewChild.children) {
        for (let child of viewChild.children) {
          if (
            (child.isInteraction() || child.isInteractionNode) &&
            child.getValue() === selectedCell.getValue()
          ) {
            cells.push(child)
          }
        }
    ...
    try {
      ...
      if (prevURI != info.getFullURI()) {
        ...
        if (this.getFromInteractionDict(prevURI)) this.removeFromInteractionDict(prevURI)
   ...
   */
  protected syncCircuitContainer(cell: typeof mxCell) {
    let toReplace = []
    const cell1 = this.graph.getModel().getCell('1')
    ...
    for (let viewChild of cell1.children) {
      if (viewChild.children) {
        for (let containerChild of viewChild.children) {
          if (
            containerChild.isCircuitContainer() &&
            containerChild.getId() != cell.getId() &&
            containerChild.getValue() === cell.getValue()
          ) {
  ...
  protected updateInteractions(oldReference: string, newReference: string) {
    try {
      ...
      for (let viewCell of viewCells) {
        if (viewCell.isComponentView()) continue
  ...
  protected async promptDeCouple(): Promise<String> {
    ...
    return new Promise(
      () => 'Other components are coupled with this one. Would you like to keep them coupled?'
    )
  }

  protected async promptChooseFunctionalComponent(
    cell: typeof mxCell,
    from: boolean
  ): Promise<string> {
    let viewCell
  ...
  protected getDefaultNewCellCoords() {
    ...
    while (true) {
      ...
      if (!centeredCells || centeredCells.length === 0) break
   ...
   * @param name
   */
  protected mutateInteractionGlyph(name: string, cell: typeof mxCell) {
    this.graph.getModel().beginUpdate()
    try {
      if (
  ...
  protected mutateInteractionNodeGlyph(name: string, cell: typeof mxCell) {
    this.graph.getModel().beginUpdate()
    try {
  ...
  protected mutateSequenceFeatureGlyph(name: string, cells: typeof mxCell)
  protected mutateSequenceFeatureGlyph(name: string, cells: typeof mxCell, graph: typeof mxGraph)
  protected mutateSequenceFeatureGlyph(
    name: string,
    cells?: typeof mxCell,
  ...
  protected makeGeneralDragsource(element, insertFunc) {
    ...
    const ds: typeof mxDragSource = mx.mxUtils.makeDraggable(
      element,
      this.graph,
      insertFunc,
      null,
      ...
      if (allGraphCells[i].children) {
        for (let j = 0; j < allGraphCells[i].children.length; j++) {
          if (allGraphCells[i].children[j].isScar()) {
            if (allGraphCells[i].children[j].getGeometry().width > 0) return true
          }
        }
      }
    }
  ...
  protected updateAngularMetadata(cells) {
    ...
    if (!this.graph.getCurrentRoot()) return
    ...
    if ((!cell && this.graph.getCurrentRoot().isModuleView()) || (cell && cell.isModule())) {
      let moduleInfo
      if (!cell) moduleInfo = this.getFromInfoDict(this.graph.getCurrentRoot().getId())
      else moduleInfo = this.getFromInfoDict(cell.value)
      if (moduleInfo) {
        this.metadataService.setSelectedModuleInfo(moduleInfo.makeCopy())
      }
    } else if (
      (!cell && this.graph.getCurrentRoot().isComponentView()) ||
      (cell &&
        (cell.isSequenceFeatureGlyph() ||
          cell.isMolecularSpeciesGlyph() ||
          cell.isCircuitContainer()))
    ) {
      let glyphInfo
      if (!cell) glyphInfo = this.getFromInfoDict(this.graph.getCurrentRoot().getId())
      else glyphInfo = this.getFromInfoDict(cell.value)
      if (glyphInfo) {
    ...
    if (cell && cell.isSequenceFeatureGlyph()) {
      let combinatorialInfo: CombinatorialInfo = this.getCombinatorialWithTemplate(
  ...
  protected getClosestCircuitContainerToPoint(x, y) {
    if (!this.graph.getDefaultParent().children) return null

    let candidates = this.graph
      .getDefaultParent()
      .children.filter((cell) => cell.isCircuitContainer())
    ...
    while (toExpand.size > 0) {
      ...
      if (viewChildren) {
        for (let child of viewChildren) {
          ...
          if (!child.isCircuitContainer() && !child.isModule()) continue
          if (child.isModule()) {
            if (reached.has(child.value)) continue
            toExpand.add(child.value)
          } else if (child.isCircuitContainer()) {
            let glyphs = this.graph.getModel().getChildren(child)
            for (let glyph of glyphs) {
              if (!glyph.isSequenceFeatureGlyph() || reached.has(glyph.value)) continue
  ...
  protected trimUnreferencedCombinatorials() {
    ...
    for (let combKey in cell0.value[GraphBase.COMBINATORIAL_DICT_INDEX]) {
      ...
      for (let cellKey in this.graph.getModel().cells) {
        ...
        if (!cell.isCircuitContainer()) continue
  ...
  protected getCoupledGlyphs(glyphURI: string): typeof mxCell[] {
    const coupledCells = []
    for (let key in this.graph.getModel().cells) {
      const cell = this.graph.getModel().cells[key]
  ...
  protected getCoupledCircuitContainers(
    glyphURI: string,
    onlyModuleContainers: boolean = false
  ): typeof mxCell[] {
    const coupledCells = []
    const cell1 = this.graph.getModel().getCell('1')
    for (let viewCell of cell1.children) {
      if (onlyModuleContainers && viewCell.isComponentView()) continue
      for (let circuitContainer of viewCell.children) {
        if (!circuitContainer.isCircuitContainer()) continue
        if (circuitContainer.value === glyphURI) coupledCells.push(circuitContainer)
      }
    }
    return coupledCells
  }
  ...
  protected getCoupledMolecularSpecies(glyphURI: string) {
    ...
    for (let viewCell of cell1.children) {
      if (viewCell.isComponentView()) continue
      for (let viewChild of viewCell.children) {
        if (!viewChild.isMolecularSpeciesGlyph()) continue
        if (viewChild.value === glyphURI) coupledCells.push(viewChild)
      }
    }
    return coupledCells
  }
  ...
  protected getCoupledModules(moduleURI: string) {
    ...
    for (let viewCell of cell1.children) {
      ...
      if (viewCell.children) {
        for (let viewChild of viewCell.children) {
          if (!viewChild.isModule()) continue
          if (viewChild.value === moduleURI) coupledCells.push(viewChild)
        }
      }
    }
    return coupledCells
   ...
   * @param newGlyphURI
   */
  protected updateViewCell(cell: typeof mxCell, newGlyphURI: string) {
  ...
  protected removeViewCell(viewCell: typeof mxCell) {
    ...
    if (viewCell.children) {
      for (let viewChild of viewCell.children) {
        if (viewChild.isModule() && this.getCoupledModules(viewChild.value).length == 0) {
          ...
          for (let containerChild of viewChild.children) {
            if (
              containerChild.isSequenceFeatureGlyph() &&
              this.getCoupledGlyphs(containerChild.value).length == 0
            )
    ...
    try {
      ...
      while (toCheck.size > 0) {
        ...
        if (!combinatorialMode) {
          ...
          for (let key in cell0.value[GraphBase.COMBINATORIAL_DICT_INDEX]) {
            const combinatorial: CombinatorialInfo =
              cell0.value[GraphBase.COMBINATORIAL_DICT_INDEX][key]
      ...
      if (!combinatorialMode) {
        ...
        if (rootViewInfo) {
          ...
          this.graph
            .getModel()
            .execute(new GraphEdits.zoomEdit(this.graph.getView(), newRootViewCell, this))
        }
        // re zoom to fix the view
        for (let i = 0; i < zoomedCells.length; i++) {
          this.graph
            .getModel()
            .execute(new GraphEdits.zoomEdit(this.graph.getView(), zoomedCells[i], this))
        }
      }
    } finally {
      this.graph.getModel().endUpdate()
  ...
  protected filterSelectionCells() {
    ...
    for (let i = 0; i < selectionCells.length; i++) {
      if (
        selectionCells[i].getParent() &&
        selectionCells[i].getParent().getParent() === this.viewStack[this.viewStack.length - 1]
      ) {
   ...
   */
  protected checkCycleUp(cell: typeof mxCell, glyphURI: string) {
    let checked = new Set<string>()
    let toCheck = new Set<string>()
   ...
   * @param glyphURI The URI you would like to set the current cell to.
   */
  protected checkCycleDown(cell: typeof mxCell, glyphURI: string) {
    let checked = new Set<string>()
    let toCheck = new Set<string>()
  ...
  protected updateInfoDict(info: Info) {
    ...
    this.graph
      .getModel()
      .execute(
        new GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[GraphBase.INFO_DICT_INDEX][info.getFullURI()]
        )
      )
  }
  ...
  protected removeFromInfoDict(glyphURI: string) {
    ...
    this.graph
      .getModel()
      .execute(
        new GraphEdits.infoEdit(cell0, null, cell0.value[GraphBase.INFO_DICT_INDEX][glyphURI])
      )
  }
    ...
    this.graph
      ...
      .execute(
        new GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[GraphBase.COMBINATORIAL_DICT_INDEX][info.getFullURI()],
          GraphBase.COMBINATORIAL_DICT_INDEX
        )
      )
  }
    ...
    this.graph
      ...
      .execute(
        new GraphEdits.infoEdit(
          cell0,
          null,
          cell0.value[GraphBase.COMBINATORIAL_DICT_INDEX][glyphURI],
          GraphBase.COMBINATORIAL_DICT_INDEX
        )
      )
  }
  ...
  protected addToCombinatorialDict(info: CombinatorialInfo) {
    ...
    this.graph
      .getModel()
      .execute(new GraphEdits.infoEdit(cell0, info, null, GraphBase.COMBINATORIAL_DICT_INDEX))
  }
    ...
    this.graph
      ...
      .execute(
        new GraphEdits.infoEdit(
          cell0,
          info,
          cell0.value[GraphBase.INTERACTION_DICT_INDEX][info.getFullURI()],
          GraphBase.INTERACTION_DICT_INDEX
        )
      )
  }
    ...
    this.graph
      ...
      .execute(
        new GraphEdits.infoEdit(
          cell0,
          null,
          cell0.value[GraphBase.INTERACTION_DICT_INDEX][interactionURI],
          GraphBase.INTERACTION_DICT_INDEX
        )
      )
  }
  ...
  protected addToInteractionDict(info: InteractionInfo) {
    ...
    this.graph
      .getModel()
      .execute(new GraphEdits.infoEdit(cell0, info, null, GraphBase.INTERACTION_DICT_INDEX))
  }
  ...
  protected createViewCell(uri: string, module: boolean = false): typeof mxCell {
    // construct the view cell
    const cell1 = this.graph.getModel().getCell(1)
    let childViewCell
    if (module) {
      childViewCell = this.graph.insertVertex(
        cell1,
        uri,
        '',
        0,
      ...
      childViewCell = this.graph.insertVertex(
        ...
        0,
        0,
        GraphBase.STYLE_COMPONENT_VIEW
      )

      // add the backbone to the view cell
      const childCircuitContainer = this.graph.insertVertex(
        childViewCell,
        null,
        uri,
        0,
        ...
        0,
        0,
        GraphBase.STYLE_CIRCUIT_CONTAINER
      )
      const childCircuitContainerBackbone = this.graph.insertVertex(
        childCircuitContainer,
        null,
        '',
        0,
        0,
